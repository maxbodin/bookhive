...
export default function AuthForm() {
  ...
  const getTitle = () => {
    if (mode === "signup") return t( "title_signup" );
    if (mode === "forgot_password") return t( "title_recovery" );
    return t( "title_signin" );
  };
  ...
  return (
    <Card className="flex flex-col w-full max-w-lg items-center justify-center p-12 border-muted mt-8 mb-12">
      ...
      <CardContent className="p-6 w-full">
        <AuthFormContent key={ mode } mode={ mode } setMode={ setMode }/>
      </CardContent>
    </Card>
  );
}